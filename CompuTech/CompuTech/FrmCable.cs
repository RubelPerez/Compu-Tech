...
namespace CompuTech
{
    public partial class FrmCable : Form
    {
        int cancel;
        public FrmCable()
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                if (cancel != 1)
                {
                    ...
                    SqlCommand cmd = new SqlCommand("insert into red (cable_maquina,cable_precio) values ('" + textBox1.Text + "','" + textBox2.Text + "')", conn);
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    conn.Close();
                ...
                else if (cancel == 1) { MessageBox.Show("CABLE YA EXISTE"); }
            }
            catch (Exception ex)
            {
