    ...
    public partial class FrmCompra : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                if (cancel != 1)
                {
                    ...
                    SqlCommand cmd = new SqlCommand("insert into almacen (alm_nombre,alm_precio,alm_obra) values ('" + textBox1.Text + "','"+textBox2.Text+"','"+textBox3.Text+"')", conn);
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    conn.Close();
                ...
                else if (cancel == 1) { MessageBox.Show("ARTICULO YA EXISTE"); }
            }
            catch (Exception ex)
            {
      ...
      WHERE alm_nombre = @ct_correo";
            ...
            if (count == 0)
            {
                ...
                cancel = 2;
                this.toolTip1.SetToolTip(pictureBox1, "Articulo exitoso");
            }
            else
            {
                pictureBox1.ImageLocation = @"D:\Programacion\400-iconos-varios-programacion\302-iconos\nuevos_iconos\varios\stop16.ico";

                this.toolTip1.SetToolTip(pictureBox1, "Ya existe este Articulo");
                cancel = 1;
