        ...
        private void button1_Click(object sender, EventArgs e)
        {
             try
             {
                 if (cancel != 1)
                 {
                     ...
                     SqlCommand cmd = new SqlCommand("insert into combox (modelo) values ('" + textBox1.Text + "')", conn);
                     conn.Open();
                     cmd.ExecuteNonQuery();
                     conn.Close();
                 ...
                 else if (cancel == 1) { MessageBox.Show("MODELO YA EXISTE"); }
            }
