    ...
    public partial class FrmProducto : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                if (cancel != 1)
                {
                    ...
                    SqlCommand cmd = new SqlCommand("insert into combox (producto) values ('" + textBox1.Text + "')", conn);
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    conn.Close();
                ...
                else if (cancel == 1) { MessageBox.Show("PRODUCTO YA EXISTE"); }
            }
      ...
      WHERE producto = @ct_correo";
            ...
            if (count == 0)
            {
                ...
                cancel = 2;
                this.toolTip1.SetToolTip(pictureBox1, "Producto exitoso");
            }
            else
            {
                pictureBox1.ImageLocation = @"D:\Programacion\400-iconos-varios-programacion\302-iconos\nuevos_iconos\varios\stop16.ico";

                this.toolTip1.SetToolTip(pictureBox1, "Ya existe esta Producto");
                cancel = 1;
