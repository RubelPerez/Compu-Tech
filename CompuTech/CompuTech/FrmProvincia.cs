    ...
    public partial class FrmProvincia : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                if (cancel != 1)
                {
                    ...
                    SqlCommand cmd = new SqlCommand("insert into combox (provincia) values ('" + textBox1.Text + "')", conn);
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    conn.Close();
                ...
                else if (cancel == 1) { MessageBox.Show("PROVINCIA YA EXISTE"); }
            }
      ...
      WHERE provincia = @ct_correo";
            ...
            if (count == 0)
            {
                ...
                cancel = 2;
                this.toolTip1.SetToolTip(pictureBox1, "Provincia exitoso");
            }
            else 
            {
                pictureBox1.ImageLocation = @"D:\Programacion\400-iconos-varios-programacion\302-iconos\nuevos_iconos\varios\stop16.ico";

                this.toolTip1.SetToolTip(pictureBox1, "Ya existe esta Provincia");
                cancel = 1;
